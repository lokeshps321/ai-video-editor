...
# Sarvam Translate accepts all scheduled Indian languages plus English.  Keep this
# mapping local to B-roll because a transcript can be generated by Whisper, Groq,
# or Sarvam and its language label is not guaranteed to use one format.
_SARVAM_TRANSLATE_LANGUAGE_CODES: dict[str, str] = {
    "as": "as-IN",
    "assamese": "as-IN",
    "bn": "bn-IN",
    "bengali": "bn-IN",
...
def _normalized_language_hint(language_hint: str | None) -> str:
    raw = str(language_hint or "").strip().lower().replace("_", "-")
    if not raw:
        return ""
...
def _sarvam_source_language_code(language_hint: str | None) -> str | None:
    """Resolve a transcript language label to a Sarvam source-language code."""
    normalized = _normalized_language_hint(language_hint)
    if normalized in {"", "auto", "detect", "default", "en", "english"}:
        return None
    return _SARVAM_TRANSLATE_LANGUAGE_CODES.get(normalized)


def _sarvam_translate_api_key() -> str:
    return (
        os.getenv("BROLL_SARVAM_API_KEY", "")
...
def _sarvam_translate_enabled(language_hint: str | None) -> bool:
    return bool(
        _env_enabled("BROLL_SARVAM_TRANSLATE_ENABLED", True)
...
def _translate_indic_text_to_english(
    *, text: str, language_hint: str | None
) -> dict[str, str] | None:
    """Translate a known Indic transcript beat before visual planning.

    A B-roll planner is good at selecting a scene but should not be the source
    of truth for Kannada, Tamil, Hindi, or other Indic-language meaning.  This
    dedicated translation call is deliberately skipped for English, unknown
    language labels, and when Sarvam has not been configured.
    """
    ...
    if not (
        ...
        and cleaned_text
        and _sarvam_translate_enabled(language_hint)
    ):
        return None
    api_url = (
        os.getenv("BROLL_SARVAM_TRANSLATE_API_URL", "")
        or "https://api.sarvam.ai/translate"
    ).strip()
    model = (
        os.getenv("BROLL_SARVAM_TRANSLATE_MODEL", "sarvam-translate:v1")
        or "sarvam-translate:v1"
    ).strip()
    translation_mode = (
        os.getenv("BROLL_SARVAM_TRANSLATE_MODE", "formal") or "formal"
    ).strip()
    timeout_sec = max(
        5.0, float(os.getenv("BROLL_SARVAM_TRANSLATE_TIMEOUT_SEC", "20") or "20")
    )
    try:
        with httpx.Client(timeout=httpx.Timeout(timeout_sec)) as client:
            response = client.post(
                api_url,
                headers={
                    "api-subscription-key": _sarvam_translate_api_key(),
                    "Content-Type": "application/json",
                },
                json={
                    "input": cleaned_text[:2000],
        ...
        logger.warning("Sarvam B-roll translation failed: %s", exc)
        return None
    translated = _normalize_short_text(
        payload.get("translated_text") if isinstance(payload, dict) else ""
    )
    if not _looks_like_valid_english_search_text(translated):
        logger.warning("Sarvam B-roll translation returned unusable text")
        return None
    return {
...
def _broll_llm_provider() -> str:
    configured = (os.getenv("BROLL_RETRIEVAL_PROVIDER", "") or "").strip().lower()
    if configured in {"gemini", "openai", "groq"}:
        return configured
    return "gemini" if _gemini_api_key() else "openai"
...
def _llm_enabled() -> bool:
    if not _env_enabled("BROLL_LLM_ENABLED", True):
        return False
    if _broll_llm_provider() == "gemini":
        return bool(_gemini_api_key() or _api_key())
...
def infer_broll_domain_context(
    *, transcript_text: str, asset_filenames: list[str]
    ...
    if not parsed:
        return fallback
    parsed.pop("__broll_planner_provider", None)
...
def _gemini_chat_json(prompt: dict[str, Any]) -> dict[str, Any] | None:
    """Run a structured B-roll planning prompt through Gemini's text API."""
    ...
    payload = {
        "systemInstruction": {
            "parts": [
                {
                    "text": (
                        "You are a B-roll retrieval assistant. Return only valid JSON."
                    )
                }
            ]
        },
    ...
    try:
        ...
        logger.warning(
            "Gemini B-roll chat completion failed with HTTP %s",
            exc.response.status_code,
        )
        return None
    except Exception as exc:
        logger.warning(
            "Gemini B-roll chat completion failed: %s", type(exc).__name__
        )
        return None


def _openai_chat_json(prompt: dict[str, Any]) -> dict[str, Any] | None:
    if not _api_key():
        return None
    payload = {
        "model": _model(),
...
def _chat_json(prompt: dict[str, Any]) -> dict[str, Any] | None:
    """Use Gemini for B-roll planning, with the existing provider as fallback."""
    if not _llm_enabled():
        return None
    if _broll_llm_provider() == "gemini":
        gemini_result = _gemini_chat_json(prompt)
        if gemini_result:
            return {
                **gemini_result,
                "__broll_planner_provider": _gemini_model(),
            }
        if _api_key():
            logger.warning("Gemini B-roll planning failed; falling back to OpenAI-compatible provider")
            fallback_result = _openai_chat_json(prompt)
            if fallback_result:
                return {
                    **fallback_result,
                    "__broll_planner_provider": _model(),
                }
            return None
        return None
    result = _openai_chat_json(prompt)
    if not result:
        return None
    return {**result, "__broll_planner_provider": _model()}
...
def _build_cross_lingual_visual_gloss(
    *,
    ...
    if translation:
        english_translation = str(translation["english_text"])
        fallback = {
            **fallback,
            "english_gloss": english_translation,
            "english_search_concept": english_translation,
            "english_query_hints": _dedupe_strings(
                [
                    english_translation,
                    f"{english_translation} cinematic",
                    *[str(item) for item in fallback["english_query_hints"]],
                ],
                limit=max_queries,
            ),
            "rationale": "Translated the Indic transcript with Sarvam before B-roll planning.",
            "translation_provider": str(translation["provider"]),
            "translation_source_language": str(translation["source_language_code"]),
        }
    if not _llm_enabled():
        return fallback
    if translation:
        prompt = {
            "goal": "Turn a verified English Indic-language translation into B-roll meaning and stock-video search ideas.",
            "source_language": str(translation["source_language_code"]),
            "original_beat_text": chunk_text[:700],
            "verified_english_translation": str(translation["english_text"]),
            "visual_intent": visual_intent,
            "domain_context": domain_context,
            "fallback_gloss": fallback,
            "rules": [
                "Treat verified_english_translation as the source of truth. Do not translate or reinterpret the original Indic text.",
                "For lyrics, choose the emotional or visual meaning of the verified translation, not a rigid word-for-word shot.",
        ...
        # Non-Indic or unconfigured translation sources retain the previous
        # generic cross-lingual handling instead of silently failing B-roll.
        prompt = {
    ...
    if not parsed:
        return fallback
    planner_provider = _normalize_short_text(
        parsed.pop("__broll_planner_provider", "")
    )
    ...
    if translation:
        result["translation_provider"] = str(translation["provider"])
        result["translation_source_language"] = str(
