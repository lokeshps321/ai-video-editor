...
from ..transliteration_service import (
...
_FILLER_SINGLE_WORDS_CONSERVATIVE: set[str] = {
    "um",
    "uh",
    "uhm",
    "umm",
...
_FILLER_SINGLE_WORDS_AGGRESSIVE: set[str] = {
    "like",
    "basically",
    "literally",
    "actually",
...
def _normalize_requested_language(value: object | None) -> str | None:
    if value is None:
        return None
...
def _detect_filler_ranges(words: list[TranscriptWord]) -> list[tuple[float, float]]:
    ...
    while idx < len(words):
        ...
        for phrase in _FILLER_MULTI_WORD_PHRASES:
            ...
            ranges.append(
                (
                    float(words[idx].start_sec),
                    float(words[idx + phrase_len - 1].end_sec),
                )
            )
...
def _pick_video_asset(
    session: Session, *, project_id: str, asset_id: str | None
) -> MediaAsset:
    ...
    if not row:
        raise HTTPException(
            status_code=404, detail="No video asset found for this project"
        )
    return row
...
def _coerce_client_words(
    raw_words: object, duration_sec: float
) -> list[TranscriptWord]:
...
def _latest_transcript(
    session: Session, *, project_id: str, asset_id: str
) -> Transcript | None:
    return session.exec(
        select(Transcript)
...
def _get_or_create_transcript(
    ...
    *,
    ...
    allow_mock_fallback = _env_bool("TRANSCRIBE_ALLOW_MOCK_FALLBACK", False)
    existing = _latest_transcript(session, project_id=project_id, asset_id=asset.id)
    normalized_requested_language = _normalize_requested_language(requested_language)
    if existing and existing.is_mock and not allow_mock_fallback:
        existing = None
    if (
        existing
        and normalized_requested_language
        and existing.language != normalized_requested_language
    ):
        existing = None
    if existing:
        existing_words = _load_words(existing)
        if existing_words:
            regenerate_low_quality = _env_bool(
                "TRANSCRIBE_REGENERATE_LOW_QUALITY", True
            )
            if not regenerate_low_quality or len(existing_words) >= _min_expected_words(
                duration_sec
            ):
                return existing
    try:
        ...
        try:
            payload = generate_transcript(
                ...
                fast_mode=fast_mode,
                language_hint=normalized_requested_language,
            )
        except TypeError as exc:
            exc_text = str(exc)
            if (
                "allow_mock_fallback" not in exc_text
                and "fast_mode" not in exc_text
                and "language_hint" not in exc_text
            ):
                raise
            try:
                payload = generate_transcript(
                ...
                if "language_hint" not in str(fallback_exc):
                    raise
...
def _apply_single_operation(
    session: Session, *, project_id: str, operation: OperationPayload
):
...
def _ensure_selected_asset_on_timeline(
    session: Session, *, project_id: str, asset_id: str, duration_sec: float
) -> None:
    timeline = get_timeline_row(session, project_id)
    state = load_timeline_state(timeline)
    video_track = next((track for track in state.tracks if track.kind == "video"), None)
    has_selected_clip = bool(
        video_track and any(clip.asset_id == asset_id for clip in video_track.clips)
    )
...
def _keep_ranges_from_silence(
    duration_sec: float, silences: list[tuple[float, float]], *, min_silence_sec: float
) -> list[dict[str, float]]:
    if not silences:
    ...
    if duration_sec > cursor + 0.08:
        ranges.append(
            {"start_sec": round(cursor, 3), "end_sec": round(duration_sec, 3)}
        )
...
def apply_vibe_action(
    ...
    if payload.action == "add_subtitles":
        ...
        requested_language = _normalize_requested_language(
            payload.options.get("transcript_language")
        )
        # Transliteration option: convert Indian scripts to English letters (romanization)
        use_transliteration = payload.options.get("transliterate", False)
        if client_words:
            words = client_words
            transcript_row = _latest_transcript(
                session, project_id=project_id, asset_id=asset.id
            )
        else:
        ...
        # Apply transliteration if requested (Indian script → Roman letters)
        word_dicts = [word.model_dump() for word in words]
        if use_transliteration:
            # Check if content has Indic script
            sample_text = " ".join(str(w.get("text", "")) for w in word_dicts[:30])
            if contains_indic_script(sample_text):
                word_dicts = transliterate_words(word_dicts)
        ...
        if requested_language:
            op_params["transcript_language"] = requested_language
        ...
        if "max_caption_overlays" in payload.options:
            op_params["max_caption_overlays"] = payload.options.get(
                "max_caption_overlays"
            )
        else:
            # Safety cap for long renders: too many drawtext filters can be OOM-killed.
            op_params["max_caption_overlays"] = max(
                60, min(140, int(duration_sec * 0.45))
            )
        op = OperationPayload(
        ...
        timeline_state = _apply_single_operation(
            session, project_id=project_id, operation=op
        )
        caption_count = 0
        ...
        for track in timeline_state.tracks:
            ...
            for clip in track.clips:
                ...
                for overlay in overlays:
                    absolute_start = max(
                        float(clip.timeline_start_sec) + float(overlay.start_sec), 0.0
                    )
        ...
        silences = detect_silence_ranges(
            source_path, noise_db=threshold, min_silence_sec=min_silence_sec
        )
        ...
        ranges = _keep_ranges_from_silence(
            duration_sec, all_pauses, min_silence_sec=min(min_silence_sec, 0.1)
        )
        op = OperationPayload(
            op_type="replace_video_track_clips",
            source="ui",
        ...
        filler_note = (
            f" Removed {len(filler_ranges)} filler word(s)." if filler_ranges else ""
        )
