...
def get_settings() -> Settings:
    return Settings(
        ...
        render_video_encoder=os.getenv("RENDER_VIDEO_ENCODER", "auto").strip().lower() or "auto",
        ...
        broll_external_max_queries=max(1, min(8, int(os.getenv("BROLL_EXTERNAL_MAX_QUERIES", "4")))),
        broll_external_download_max_mb=max(5, int(os.getenv("BROLL_EXTERNAL_DOWNLOAD_MAX_MB", "512"))),
        ...
        broll_shortform_min_sec=max(0.2, float(os.getenv("BROLL_SHORTFORM_MIN_SEC", "0.8"))),
        broll_shortform_max_sec=max(0.4, float(os.getenv("BROLL_SHORTFORM_MAX_SEC", "2.0"))),
        broll_audio_reactive_enabled=_as_bool(os.getenv("BROLL_AUDIO_REACTIVE_ENABLED", "true"), True),
        broll_audio_reactive_window_sec=max(0.05, float(os.getenv("BROLL_AUDIO_REACTIVE_WINDOW_SEC", "0.24"))),
        broll_audio_reactive_sample_rate=max(4000, int(os.getenv("BROLL_AUDIO_REACTIVE_SAMPLE_RATE", "11025"))),
        broll_auto_reframe_enabled=_as_bool(os.getenv("BROLL_AUTO_REFRAME_ENABLED", "true"), True),
        broll_generative_enabled=_as_bool(os.getenv("BROLL_GENERATIVE_ENABLED", "false"), False),
        ...
        broll_generative_timeout_sec=max(2.0, float(os.getenv("BROLL_GENERATIVE_TIMEOUT_SEC", "30"))),
