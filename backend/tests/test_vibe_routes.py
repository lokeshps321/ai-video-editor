...
def test_vibe_add_subtitles_regenerates_when_latest_transcript_is_mock(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TRANSCRIBE_ALLOW_MOCK_FALLBACK", "false")
    monkeypatch.setenv("TRANSCRIBE_REGENERATE_LOW_QUALITY", "false")
    monkeypatch.setattr("app.routers.media.probe_duration_seconds", lambda _: 8.0)
    monkeypatch.setattr("app.routers.media.probe_stream_flags", lambda _: {"has_video": True, "has_audio": True})
    monkeypatch.setattr("app.routers.vibe.detect_silence_ranges", lambda *_args, **_kwargs: [])
    monkeypatch.setattr("app.routers.vibe.enqueue_render_job", lambda *_args, **_kwargs: None)
    ...
    def fake_generate(_path: str, _duration_sec: float, **kwargs) -> TranscriptPayload:
        ...
        words = [
            ...
            TranscriptWordPayload(id="rw2", text="captions", start_sec=0.6, end_sec=1.0),
        ]
        return TranscriptPayload(source="test", language="en", text="real captions", words=words, is_mock=False)
    ...
    with TestClient(app) as client:
        ...
        mock_words = [
            {"id": f"mw{idx}", "text": f"mock{idx}", "start_sec": idx * 0.4, "end_sec": (idx * 0.4) + 0.2}
            for idx in range(12)
        ]
        with Session(engine) as session:
            mock_row = Transcript(
...
def test_vibe_add_subtitles_sanitizes_existing_pathological_word_timings(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TRANSCRIBE_REGENERATE_LOW_QUALITY", "false")
    monkeypatch.setattr("app.routers.media.probe_duration_seconds", lambda _: 8.0)
    monkeypatch.setattr("app.routers.media.probe_stream_flags", lambda _: {"has_video": True, "has_audio": True})
    monkeypatch.setattr("app.routers.vibe.detect_silence_ranges", lambda *_args, **_kwargs: [])
    monkeypatch.setattr("app.routers.vibe.generate_transcript", lambda *_args, **_kwargs: pytest.fail("unexpected regenerate"))
...
def test_vibe_add_subtitles_uses_client_supplied_words(monkeypatch: pytest.MonkeyPatch) -> None:
            ...
            json={
                ...
                "options": {
                    "words": [
                        ...
                        {"id": "c2", "text": "caption", "start_sec": 0.8, "end_sec": 1.1},
                    ]
                },
            },
        )
...
def test_vibe_auto_cut_pauses_uses_conservative_filler_detection_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_vibe_auto_cut_pauses_respects_string_false_for_remove_fillers(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("app.routers.media.probe_duration_seconds", lambda _: 8.0)
    monkeypatch.setattr("app.routers.media.probe_stream_flags", lambda _: {"has_video": True, "has_audio": True})
    monkeypatch.setattr("app.routers.vibe.detect_silence_ranges", lambda *_args, **_kwargs: [])
    ...
    def fake_generate(_path: str, _duration_sec: float) -> TranscriptPayload:
        ...
        return TranscriptPayload(source="test", language="en", text="um hello", words=words, is_mock=False)
